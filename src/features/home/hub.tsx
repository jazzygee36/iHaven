"use client";
import HomeButton from "@/components/button";
import Image from "next/image";

const hubs = [
  {
    title: "Kaduna Tech Hub",
    description:
      "Connecting youth and developers in Northern Nigeria with free meetups, workshops, and mentorship.",
    image:
      "https://images.unsplash.com/photo-1497366216548-37526070297c?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&h=600",
  },
  {
    title: "Kano Innovation Center",
    description:
      "Our Kano community is growing fast — join weekly sessions on design, web, and entrepreneurship.",
    image:
      "https://images.unsplash.com/photo-1497366216548-37526070297c?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&h=600",
  },
  {
    title: "Kastina Creators Space",
    description:
      "Our capital-based hub where digital creators collaborate and build real-world solutions.",
    image:
      "https://images.unsplash.com/photo-1497366216548-37526070297c?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&h=600",
  },
];

const CommunityHubs = () => {
  return (
    <section className="bg-gradient-to-b from-[#fdfbfb] to-[#ebedee] py-16">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="text-[25px] md:text-3xl font-bold text-gray-900 text-center mb-6">
          Our Community <span className="text-[#FF6933]">Hubs</span>
        </h2>
        <p
          className="text-center text-gray-700 max-w-2xl mx-auto mb-12 text-[16px]"
          data-aos="fade-up"
        >
          We're building physical and virtual spaces to empower developers,
          designers, and digital creators in every region. Join a hub near you
          and grow with others.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {hubs.map((hub, idx) => (
            <div
              key={idx}
              className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 group"
            >
              <div className="relative h-52 w-full">
                <Image
                  src={hub.image}
                  alt={hub.title}
                  fill
                  className="object-cover group-hover:scale-105 transition-transform duration-300"
                />
              </div>

              <div className="p-5 space-y-3">
                <h3 className="text-lg font-semibold text-gray-800">
                  {hub.title}
                </h3>
                <p className="text-gray-600 text-sm">{hub.description}</p>

                <HomeButton
                  title={"Explore More →"}
                  type={"button"}
                  bg={"transparent"}
                  width={""}
                  height={""}
                  color="#FF6933"
                  // hoverEffect="underline"
                />
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default CommunityHubs;
